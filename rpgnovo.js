...
function criarPersonagem() {
  ...
  const classe = parseInt(readline.question("Opcao: "));
  ...
  switch (escolha) {
    case 1:
      ...
      while (inimigo.vida > 0 && personagem.vida > 0) {
        ...
        if (acao === 1) {
          // vida do inimigo não fica menor que zero, aparece quantro de vida resta para ambos
          ...
          console.log(
            `Vida restante do ${inimigo.nome}: ${inimigo.vida}/${inimigo.vidafixa}`
          );
        } else if (acao === 2) {
          console.log("Você se defendeu e recebeu menos dano!");
          personagem.defesa *= 1.5;
          console.log(
            `Vida do ${inimigo.nome}: ${inimigo.vida}/${inimigo.vidafixa}`
          );
        } else if (acao === 3) {
          // Adicionando a fuga do personagem com chance de fracasso e penalidade.
          ...
          if (chanceEscape > 40) {
            ...
            console.log(
              `Você tentou escapar e tropeçou numa pedra e o ${inimigo.nome} te desferiu um ataque!`
            );
          }
        }
        ...
        if (inimigo.vida > 0) {
          ...
          console.log(
            `${inimigo.nome} causou ${danoRecebido} de dano em você.\nVida restante do jogador: ${personagem.vida}`
          ); // aparecer vida do personagem apos o ataque
        }
    ...
    case 5:
      ...
      for (let i = 0; i < missoes.length; i++) {
        console.log(
          `${i + 1}. ${missoes[i]} - ${
            missoesCompletas[i] ? "Completa" : "Incompleta"
          }`
        );
      }
      break;
      ...
      if (inimigosDerrotados >= 3) {
        ...
        while (boss.vida > 0 && personagem.vida > 0) {
          ...
          if (acaoBoss === 1) {
            ...
            // escapar boss com punição caso não consiga
            let chanceEscape = Math.random() * 100;
            if (chanceEscape > 70) {
              ...
              console.log(
                console.log(
                  `Você tentou escapar e tropeçou numa pedra e o ${boss.nome} te desferiu um ataque!`
                )
              );
          ...
          if (boss.vida > 0) {
            ...
            personagem.vida -= danoRecebido;
            console.log(
              `${boss.nome} causou ${danoRecebido} de dano em você.\nVida restante do jogador: ${personagem.vida}`
            );
          }
        ...
        console.log(
          "Você ainda não completou os requisitos para enfrentar o Boss."
        );
      }
      break;
